...
class LevelData {
  ...
  static LevelModel? getLevel(int levelNumber) {
    if (levelNumber < 1 || levelNumber > allLevels.length) return null;
    return allLevels[levelNumber - 1];
  }

  static bool hasLevel(int levelNumber) =>
      levelNumber >= 1 && levelNumber <= allLevels.length;

  static int? getNextLevelNumber(int currentLevel) {
    if (currentLevel < allLevels.length) return currentLevel + 1;
    return null;
  }
}
